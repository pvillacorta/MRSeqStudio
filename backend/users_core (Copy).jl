...
"""
get_user_privileges(username)

Obtiene los privilegios de un usuario específico.

# Arguments
- `username::String`: Nombre de usuario

# Returns
- `Union{Dict{String, Any}, Nothing}`: Diccionario con los privilegios del usuario o nothing si no existe
"""
function get_user_privileges(username::String)
    conn = get_db_connection()
    try
        ...
        query = """
        SELECT u.id, u.username, u.is_premium, u.is_admin,
            COALESCE(p.storage_quota_mb, 0.5) as storage_quota_mb, 
            COALESCE(p.gpu_access, 0) as gpu_access, 
            COALESCE(p.max_daily_sequences, 10) as max_daily_sequences
        FROM users u
        LEFT JOIN user_privileges p ON u.id = p.user_id
        WHERE u.username = ?
        """
        ...
        for row in result
            # Crear diccionario con los privilegios del usuario
            privileges = Dict(
                "id" => row[1],
                "username" => row[2],
                "is_premium" => row[3] == 1,
                "is_admin" => row[4] == 1,
                "storage_quota_mb" => row[5],
                "gpu_access" => row[6] == 1,
                "max_daily_sequences" => row[7]
            )
            return privileges
        end
        ...
        println("❌ Error getting user privileges: ", e)
        return nothing
    finally
        DBInterface.close!(conn)
    end
